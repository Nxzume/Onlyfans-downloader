...
class OnlyFansDownloader {
  ...
  waitForContent() {
    ...
    const checkForContent = () => {
      ...
      if (hasPosts || hasMessages || hasVideos || hasImages) {
        console.log('[OK] OnlyFans content detected, starting downloader...');
    ...
    const retry = () => {
      ...
      if (attempts < maxAttempts) {
        ...
        console.log('[WARN] Content not detected after 30 seconds, but continuing...');
        ...
        if (postId) {
          for (const [cleanUrl, data] of this.networkVideoUrls.entries()) {
            if (data.url.includes(postId)) {
              videoUrl = data.url;
              hasVideo = true;
              console.log('[OK] Using network-captured video URL for post:', postId);
              break;
            }
          }
        }
  ...
  processVideoWrappers(videoWrappers) {
    ...
    videoWrappers.forEach(wrapper => {
      ...
      if (!videoUrl) {
        console.log('[WARN] No video URL found for wrapper, will retry later');
        setTimeout(() => {
  ...
  processDimensionVideoPlayers(dimensionPlayers) {
    ...
    dimensionPlayers.forEach(player => {
      ...
      if (!videoUrl) {
        console.log('[WARN] No video URL found for dimension player, will retry later');
        setTimeout(() => {
  ...
  processVideoJsPlayers(videoJsPlayers) {
    ...
    videoJsPlayers.forEach(player => {
      ...
      if (!videoUrl) {
        console.log('[WARN] No video URL found for video.js player, will retry later');
        setTimeout(() => {
  ...
  processStandaloneVideos(standaloneVideos) {
    ...
    standaloneVideos.forEach(video => {
      ...
      if (!videoUrl) {
        console.log('[WARN] No video URL found for standalone video, will retry later');
        setTimeout(() => {
  ...
  processDataAttributeVideos(dataAttributeVideos) {
    ...
    dataAttributeVideos.forEach(video => {
      ...
      if (!videoUrl) {
        console.log('[WARN] No video URL found for data attribute video, will retry later');
        setTimeout(() => {
        ...
        if (buttonContainer) {
          ...
          if (cleanOldUrl && cleanOldUrl !== cleanNewUrl) {
            ...
            if (!buttonText.includes('🔄')) {
              btn.textContent = buttonText.replace(/🎬|Video|video/, '🎬 Download Video');
            }
  ...
  refreshButtonsOnMediaChange(post) {
    ...
    this.refreshButtonsTimeout = setTimeout(() => {
      ...
      if (foundVideoUrl && existingButtons.length === 0) {
        ...
        console.log('[WARN] No video URL found, keeping existing buttons');
      }
    }, 300); // Debounce delay
  }
